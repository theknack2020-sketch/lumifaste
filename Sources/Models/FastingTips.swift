...
enum FastingTips {
    ...
    static func stageTips(for stage: FastingStage) -> [Tip] {
        switch stage {
        case .fed:
            tips.filter { $0.category == .nutrition || $0.category == .motivation }
        case .earlyFasting:
            tips.filter { $0.category == .hydration || $0.category == .mentalClarity }
        case .fatBurning:
            tips.filter { $0.category == .exercise || $0.category == .science }
        case .ketosis:
            tips.filter { $0.category == .science || $0.category == .mentalClarity }
        case .autophagy:
            tips.filter { $0.category == .science || $0.category == .motivation }
        }
    }
}
