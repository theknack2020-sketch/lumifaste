...
final class ChallengeManager {
    ...
    func evaluate(sessions: [FastingSession]) -> [FastingChallenge] {
        ...
        let longFasts = completed.count(where: { $0.actualDuration >= 16 * 3600 })
        ...
        let earlyFasts = completed.count(where: { session in
        ...
        let todayLong = todayCompleted.count(where: { $0.actualDuration >= 16 * 3600 })
        ...
        let monthAutophagy = monthCompleted.count(where: { $0.stageReached == FastingStage.autophagy.rawValue })
