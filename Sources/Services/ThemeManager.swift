...
enum AppTheme: String, CaseIterable, Identifiable, Codable {
    ...
    var accent: Color {
        Color(uiColor: UIColor { $0.userInterfaceStyle == .dark ? self.darkAccentUI : self.lightAccentUI })
    }
    
    /// Gradient start color — adapts to light/dark
    var gradientStart: Color {
        Color(uiColor: UIColor { $0.userInterfaceStyle == .dark ? self.darkGradientStartUI : self.lightGradientStartUI })
    }
    
    /// Gradient end color — adapts to light/dark
    var gradientEnd: Color {
        Color(uiColor: UIColor { $0.userInterfaceStyle == .dark ? self.darkGradientEndUI : self.lightGradientEndUI })
    }
...
final class ThemeManager {
    ...
    init() {
        ...
        self.selectedTheme = AppTheme(rawValue: raw) ?? .defaultGreen
    }
    ...
    /// Premium-only themes
    static let premiumThemes: [AppTheme] = AppTheme.allCases.filter { $0.isPremium }
}
