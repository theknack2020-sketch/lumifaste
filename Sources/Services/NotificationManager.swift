...
final class NotificationSettings {
    ...
    init() {
        ...
        dailyReminderEnabled = ud.bool(forKey: Key.dailyReminder)
        milestoneEnabled = ud.bool(forKey: Key.milestone)
        stageTransitionEnabled = ud.bool(forKey: Key.stageTransition)
        fastCompleteEnabled = ud.bool(forKey: Key.fastComplete)
        motivationalQuotesEnabled = ud.bool(forKey: Key.motivationalQuotes)
        streakReminderEnabled = ud.bool(forKey: Key.streakReminder)
        inactivityNudgeEnabled = ud.bool(forKey: Key.inactivityNudge)
        weeklySummaryEnabled = ud.bool(forKey: Key.weeklySummary)
        morningMotivationEnabled = ud.bool(forKey: Key.morningMotivation)
        waterReminderEnabled = ud.bool(forKey: Key.waterReminder)
        eatingWindowReminderEnabled = ud.bool(forKey: Key.eatingWindowReminder)
        dailyReminderHour = ud.integer(forKey: Key.dailyReminderHour)
        dailyReminderMinute = ud.integer(forKey: Key.dailyReminderMinute)
        quietHoursEnabled = ud.bool(forKey: Key.quietHoursEnabled)
        quietHoursStart = ud.integer(forKey: Key.quietHoursStart)
        quietHoursEnd = ud.integer(forKey: Key.quietHoursEnd)
    }
...
final class NotificationManager: NSObject, UNUserNotificationCenterDelegate {
    ...
    private func streakMotivationalMessage(for streak: Int) -> String {
        switch streak {
        case 1 ... 2:
            "You're building a habit! Every day counts. Keep going 💪"
        case 3 ... 6:
            "Your body is adapting to your fasting rhythm. Consistency is key! 🌱"
        case 7 ... 13:
            "A full week and counting! Your discipline is paying off 🔥"
        case 14 ... 29:
            "Two weeks strong! You're in the top tier of fasters. Amazing willpower ⚡"
        case 30 ... 59:
            "A whole month! Your metabolism thanks you. This is extraordinary 🏆"
        case 60 ... 99:
            "Over \(streak) days — you've made fasting a lifestyle. Legendary! 👑"
        default:
            "\(streak) days! You're an absolute inspiration. Never stop 🌟"
        }
    }
    ...
    func scheduleMorningNudge(averageStartTime: String?) {
        ...
        let body = if let avgTime = averageStartTime {
            "Ready for today's fast? Your average start time is \(avgTime) — keep the rhythm going! 🍃"
        } else {
            "Ready for today's fast? Start when it feels right — every hour counts! 🍃"
        }
    ...
    func scheduleInactivityNudge(daysSinceLastFast: Int?) {
        ...
        let body = switch days {
        case 3 ... 5:
            "We miss you — even a 12h fast counts! Your body remembers the rhythm."
        case 6 ... 13:
            "It's been \(days) days since your last fast. A gentle 12:12 is all it takes to get back on track."
        default:
            "Welcome back whenever you're ready. A simple 12-hour fast is a great restart."
        }
    ...
    func scheduleWeeklySummary(stats: FastingManager.WeeklyStats, currentStreak: Int) {
        ...
        let streakPart = if currentStreak > 0 {
