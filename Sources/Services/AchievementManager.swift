...
final class AchievementManager {
    ...
    func evaluate(sessions: [FastingSession]) -> [Achievement] {
        ...
        // Stage-based
        let fatBurningCount = completed.filter { FastingStage(rawValue: $0.stageReached)?.index ?? 0 >= FastingStage.fatBurning.index }.count
        if !isEarned(.fatBurner) && fatBurningCount >= 5 {
            unlock(.fatBurner)
            newlyUnlocked.append(.fatBurner)
        }
        
        let ketosisCount = completed.filter { FastingStage(rawValue: $0.stageReached)?.index ?? 0 >= FastingStage.ketosis.index }.count
