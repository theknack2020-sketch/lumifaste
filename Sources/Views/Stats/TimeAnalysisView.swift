...
struct TimeAnalysisView: View {
    ...
    private func peakTimesRow(accent _: Color) -> some View {
        HStack(spacing: 0) {
            VStack(spacing: 4) {
        ...
        .chartLegend(position: .bottom, spacing: 8) {
            HStack(spacing: 16) {
                HStack(spacing: 4) {
                    ...
                    Text("Start").font(.adaptiveBadge(isRegular: isRegular)).foregroundStyle(.secondary)
                }
                HStack(spacing: 4) {
                    Circle().fill(.orange).frame(width: 8, height: 8)
                    Text("End").font(.adaptiveBadge(isRegular: isRegular)).foregroundStyle(.secondary)
                }
            }
        }
        .chartYAxis {
